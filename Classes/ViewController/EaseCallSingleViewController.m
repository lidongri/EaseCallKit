...
- (void)viewDidLoad {
    ...
    self.remoteHeadView.layer.cornerRadius = 40;
    self.remoteHeadView.layer.masksToBounds = YES;
...
- (void)miniAction
{
    ...
    if(self.isConnected) {
        ...
        self.floatingView.nameLabel.text = @"正在等待对方接受邀请";
        self.floatingView.enableVideo = NO;
    }
}
...
- (void)setIsConnected:(BOOL)isConnected
{
    ...
    if(isConnected)
    {
        [[NSNotificationCenter defaultCenter] postNotificationName:@"BWHomeCounselorAudioStartNotify" object:nil];
        [self startTimer];
